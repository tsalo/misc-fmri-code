function [handles] = xjviewModified(varargin)

addpath('/nfs/pkg64/contrib/xjview_8.4/');
% xjview, version 8.1
%
...
% usage 4: xjviewModified(mnicoord, intensity)
...
if length(varargin)==4
    inputXyz = varargin{4};
    yflag = 0;
    %     yflag=1; color_limit=varargin{4};
else
    yflag=0;
end
xyz_cordinates	= [-50 -16 -12];
...
commonRegionPushPosition =          stretchMatrix*[0.400,   4*heightUnit,    editBoxWidth,    editBoxHeight]' + controlPanelOffset;
...
elseif isstr(varargin{1})
    CallBack_loadImagePush(handles.loadImagePush, [], varargin{1});
else
    mniCoord = varargin{1};
...
if yflag==1
    custom_color_range(color_limit)
end
handles2 = guidata(handles.figure);
spm_XYZreg('SetCoords', inputXyz, handles2.hReg);
end
...
xjviewModified(cor2mni(coord, handles.M{1}), z);
end
...
bb      = [];
st      = struct('n', 0, 'vols',[], 'bb',bb,'Space',eye(4),'centre',[0 0 0],'callback',';','xhairs',0,'hld',1,'fig',fig,'mode',1,'plugins',[],'snap',[]);
st.vols = cell(24,1);
...
if isdir(pluginpath)
    ...
    if ~isempty(pluginfiles)
        ...
        for k = 1:length(pluginfiles)
            [pluginname] = fileparts(pluginfiles(k).name);
...
function custom_color_range(Ylim)
...
set(handles.hcolorbar,'Ylim',Ylim);
