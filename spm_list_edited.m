function varargout = spm_list_edited(varargin)
% Display an analysis of SPM{.}
% FORMAT TabDat = spm_list_edited('List',xSPM,hReg,[Num,Dis,Str])
% Summary list of local maxima for entire volume of interest
% FORMAT TabDat = spm_list_edited('ListCluster',xSPM,hReg,[Num,Dis,Str])
...
%                           ----------------
%
% FORMAT spm_list_edited('TxtList',TabDat,c)
...
%                           ----------------
%
% FORMAT spm_list_edited('SetCoords',xyz,hAx,hReg)
...
% $Id: spm_list_edited.m 4617 2012-01-11 15:46:16Z will $
...
% FORMAT TabDat = spm_list_edited('List',xSPM,hReg,[Num,Dis,Str])
    ...
    %----------------------------------------------------------------------
    TabDat = spm_list_edited('Table',xSPM,varargin{4:end});
...
case 'table'                                                        %-Table
%==========================================================================
    % FORMAT TabDat = spm_list_edited('table',xSPM,[Num,Dis,Str])
    ...
    %----------------------------------------------------------------------
    if length(varargin) > 2, Num = varargin{3}; else Num = 3; end%spm_get_defaults('stats.results.volume.nbmax'); end
